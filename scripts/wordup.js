...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            model.wordSubmissions.forEach(function(obj) {
                if (obj.word == word)
                obj.isRealWord = theAnswer})
            // re-render
            render();
        },
        error: function(err) {
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
            if (wordSubmission.isRealWord == true) {
                content = wordScore(wordSubmission.word);
            }
                else {
                    content = "X"
                }
        ...
        if (wordSubmission.isRealWord == false) {
            scoreChip.addClass("tag-sm red-back");
        }
            else {
                scoreChip.addClass("tag-sm blue-back")
            }
...
function isDisallowedLetter(letter) {
    var allowedLetters = model.allowedLetters;
    var isAllowed = allowedLetters.indexOf(letter)
